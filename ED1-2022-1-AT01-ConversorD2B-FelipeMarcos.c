...
int isnum(char c){
	...
	for (int i = 0; i < 10 && is == 0; i++){
		if (c == nums[i]) is = 1;//se for um numero retorna 1
	}
	return is;
}
...
int main(int argc, const char * argv[]) {
	...
	int num,valid = 1,ncl = 0;//ncl = Nume de caracteres por linha e um contador usado para voltar ao inicio da linha apos a validaçao
  ...
  inicia(&sub);
	...
	while (!feof(entrada)){//loop de todo o arquivo
		int lido = 0;
    num = 0;
      ...
      lido = 1;
		...
		if(!valid || !lido)break;//se for invalido  sai
		tevealgo = 1;
		if (num == 0) {
			push(&main, 0);
		} else if (num == 1) {
